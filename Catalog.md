# Pearl Memorial

## Engraved Memorials — Packages & Pricing

### Cross Boundaries Through Resonance

To exist. To act. To eternity.

**Your story matters.**

Laser-engraved keepsakes to preserve your memories.

---

## Standing at the Boundary?

**You're not alone. We've been there too.**

A **Boundarist** is someone standing at the boundary — someone who has experienced the moment when systems stop working as they "should."

Whether you're outside the system or seeing its limits from within.

| If you're... | The message for you |
|---|---|
| **Outside** (system failed you) | "It's okay to exist." |
| **Inside** (seeing limits, wanting to act) | "You can act on your own judgment." |
| **Everyone** | "Your story matters." |

## The 7 Systems We Depend On

Standing at the boundary = when these systems shake:

| System | What we depend on | When it shakes |
|---|---|---|
| **Survival** | Body, housing, energy | Illness, homelessness, power outage |
| **Social** | Family, partner, community | Bereavement, divorce, isolation |
| **Economic** | Income, assets, credit | Job loss, bankruptcy, layoffs |
| **Institutional** | Nationality, visa, qualifications | Entry denied, license revoked |
| **Information** | Communication, access | No signal, censorship |
| **Inner** | Identity, meaning, spirit | "Who am I?", burnout |
| **Temporal** | Life stage, generation | Feeling outdated, generation gap |

---

## Timeless Eternal Hope — Four Pillars

Pearl Memorial is part of **Timeless Eternal Hope**, built on four pillars:

| Pillar | What it means | How we do it |
|---|---|---|
| 🕊 **Carry** | Soul Carrier | We carry souls home |
| 🔥 **Inscribe** | Proof of existence | Laser engraving preserves memories |
| 🛡 **Preserve** | Disaster-resistant memory | Quartz glass, QR archives |
| ⚡ **Sustain** | Digital independence | Your own micro datacenter, protection on your terms |

---

## Soul Carrier — Reuniting Roots

**We carry souls home.**

People crossed the ocean. For different reasons, in different eras. Over the years, connections to their homeland faded.

### We help deliver:

- **Ashes to ancestral homes** — Return remains to family graves in Japan
- **Families to their roots** — Reconnect with relatives, registries, and history
- **Family crests to memory** — Research your kamon with EN/JP report on its history and significance
- **Stories to completion** — Close the circle that distance left open

> "Loss doesn't have to be the end."

### Why existing services struggle

It's not anyone's fault. But there are walls.

| Wall | Reality |
|---|---|
| Language | City halls and temples rarely offer English support |
| Systems | Family registries and cemetery procedures require a Japanese address |
| Continuity | No one handles research, coordination, and accompaniment as one journey |
| Heart | Few treat this as "completing a story" rather than "processing paperwork" |

We stand on both sides of these walls.

### Why Us?

**We stand at the boundary ourselves.**

| What we offer | Why it matters |
|---|---|
| Japanese natives living between Hawaii and Japan | We understand both worlds |
| Fluent in both languages and cultures | No translation barriers |
| We have homes in Japan ready to welcome you | Stay with family, not hotels |
| We know the systems: family registries, temples, cemeteries | We navigate the bureaucracy for you |
| We've experienced loss — Pearl (2007-2025) led us here | We understand grief firsthand |

Pearl was family for 18 years. After creating a portable grave for him, we visited Pearl Harbor as a family. With his remains, we prayed for peace. 80 years after the war — times have changed, but the wish for peace transcends generations. This experience awakened our family's mission.

**We don't just research. We walk with you. We carry your story home.**

### Pricing

| Service | Price | Description |
|---|---|---|
| Consultation | Free | 30-minute call to understand your situation |
| Research | $500 | Locate family records, graves, contacts, and kamon |
| Coordination | $1,000 | Arrange ceremonies, temple visits, and logistics |
| Accompaniment / Proxy | $2,000+ | We travel with you — or on your behalf (+ travel expenses, typically $1,500–$3,000) |

*Custom packages available. Contact us to discuss your needs.*

---

## Memorial Packages

### Moment — Returns to Earth
**$30–$50**

Materials: Palm leaves, driftwood, natural materials

*We come to you*

---

### Memory — A Lifetime
**$100–$200**

Materials: Wood, charcoal, coconut shell

*We come to you*

---

### Legacy — Across Generations
**$300–$500**

Materials: Lava rock, metal

*Visit our workshop or Japan*

---

### Eternal — Forever
**$5,000**

Materials: Quartz glass internal engraving (1,000+ years)

*Crafted at our Japan studio*

---

## Preserve — Places to Keep & Visit

**Your memories, protected. Your presence, welcomed.**

A place to store. A place to return.

### Urayasu — Near Tokyo
- 30 min to Tokyo
- Near Disney Resort (fireworks view)
- 4LDK Frank Lloyd Wright Organic House
- Storage + stay for visits
- Starlink Wi-Fi

### Kurihara — Countryside
- Traditional farmhouse
- All tatami rooms
- Rice paddies & nature
- Homemade meals by Mina's mother
- Storage + stay for visits
- Starlink Wi-Fi

### Hana, Maui — Pacific
- Remote east coast of Maui
- Ocean & nature
- Storage + stay for visits
- Off-grid ready

**Available:** Jan–Mar, Jun–Jul, Nov–Dec

### Storage Tiers
| Tier | Locations | Shikinen | Description |
|---|---|---|---|
| Single | 1 site | — | Urayasu or Kurihara or Hana |
| Dual | 2 sites | — | Domestic + overseas redundancy |
| Triple | 3 sites | ✓ | All sites + renewal every 20 years |

**Shikinen Renewal:** Every 20 years, data migrates to new storage media with ceremonial verification.

*Archive locations expanding. New sites announced as they come online.*

> "A place to keep. A place to return."

---

## Sustain — Your Own Micro Datacenter

**Own your data. Data protection on your terms.**

*Tested and running on Maui since 2023.*

### The Stack
- **Solar Panel** — Off-grid power
- **Jackery 290W** — Portable battery (car-mountable)
- **Starlink Mini** — Satellite internet anywhere
- **Mac Mini** — Your personal server

### Service

| Package | Price | What you get |
|---|---|---|
| Micro DC Setup | $3,000 | Equipment + setup + handover. Ready to use. |
| Managed Hosting | $100/month | We maintain your datacenter remotely |
| DIY Support | $500 | Consultation + equipment list. You build it. |

**How it works:**
1. We procure and configure the equipment
2. We set up at your location (or ship pre-configured)
3. We hand over access credentials
4. Your data lives on YOUR hardware

> "Your memories don't belong to a corporation. They belong to you."

---

## Start Yourself — Equipment & Sourcing

**The movement is replicable. You can start too.**

**Gold = The color of the movement. The symbol of "Cross boundaries through resonance."**

### Handmade Soap (for Gifting)

| Material | Price Range | Source | Notes |
|---|---|---|---|
| MP Soap Base | $15–25/kg | Amazon | Melt and pour, beginner-friendly |
| Coconut Oil | $10–15 | Costco / Grocery | Moisturizing, good lather |
| Essential Oils | $10–20 | Amazon | Lavender, tea tree, etc. |
| Food-Grade Colorant | $10–15 | Amazon | Safe, skin-friendly |
| Silicone Molds | $10–20 | Amazon / Dollar store | Cookie molds, bento cups work too |
| Mesh Drawstring Bags | $10–15/50pcs | Amazon / Dollar store | Doubles as lather pouch |
| OPP Bags | $5–10/100pcs | Amazon / Dollar store | Individual wrap, moisture protection |
| Label | $15–20 | Amazon | e.g., P-touch 18mm+, gold, clear, etc. |

**How to:** Melt MP soap in double boiler → Mix in oil, scent, color → Pour into molds → Cool → Place in mesh bag → Wrap in OPP bag → Add label → Done

**Cost per batch (~10 bars):** $5–10

### Label Message (Example)

Sample message. Feel free to customize:

> At the boundary? You're not alone.
> Cross Boundaries Through Resonance.
> To exist. To act. To eternity.
> Boundarist Movement by Pearl Memorial
> https://bit.ly/boundarist

---

### Laser Engraving Equipment

| Equipment | Price Range | Source | Use |
|---|---|---|---|
| LaserPecker 2 | $400–600 | Amazon | Wood, leather, paper (best for beginners) |
| LaserPecker 5 | $800–1,200 | Amazon | Metal & diverse materials |
| Brother P-touch Cube | $50–100 | Amazon | Label printing (gold tape compatible) |

### Off-Grid Equipment

| Equipment | Price Range | Source | Use |
|---|---|---|---|
| Jackery 1000 Plus | $1,200–1,500 | Amazon / Costco | Stationary power station (heavy) |
| Jackery Explorer 290W | $300–400 | Amazon | Car-mounted / mobile power |
| Jackery 100W Solar Panel | $300–400 | Amazon | Charging |
| Starlink Mini | $599 + $50/mo | starlink.com | Satellite internet |
| TP-Link Deco (Captive Portal model) | $150–250 | Amazon | Guest WiFi capable mesh |

### Living Infrastructure

| Equipment | Price Range | Source | Use |
|---|---|---|---|
| IGLOO Electric Cooler | $200–300 | Amazon | 12V compatible, car-friendly |
| Gravity Water Filter | $50–100 | Amazon | No power needed |
| Portable Washing Machine | $100–150 | Amazon | Compact, manual/electric |
| Propane Burner | $30–50 | Costco / Home Depot | Cooking |

### Minimum Setup (~$2,500 to start)

1. Jackery 1000 Plus + Solar Panel
2. Starlink Mini
3. LaserPecker 2
4. Soap supplies (~$50)

That's all you need to "exist, engrave, and gift" — without permission, with resonance.

---

## How To — Laser Engraving Basics

### Choosing Materials

| Material | Source | Characteristics | Difficulty |
|---|---|---|---|
| Palm leaves | Beach, garden | Returns to earth, lightweight | Easy |
| Driftwood | Beach | Natural shapes, one-of-a-kind | Easy |
| Coconut shell | Market, grocery | Hard, long-lasting | Medium |
| Lava rock | Hawaii local, garden stores | Heavy, lasts generations | Medium |
| Charcoal | BBQ supply stores | Black base, white engraving pops | Easy |
| Metal plate | Amazon, hardware stores | Permanent, outdoor-safe | Hard |

### Engraving Process

1. **Wash and dry material** (24+ hours)
2. **Flatten surface** (light sanding if needed)
3. **Create design** (Canva, Illustrator, etc.)
4. **Test engrave** (adjust power/speed on scrap)
5. **Final engrave**
6. **Finish** (oil for wood, clear coat for metal)

### Recommended Settings (LaserPecker 2 Pro)

| Material | Power | Speed | Passes |
|---|---|---|---|
| Wood | 80% | 60% | 1 |
| Palm leaf | 40% | 80% | 1 |
| Coconut shell | 100% | 40% | 2 |
| Charcoal | 30% | 90% | 1 |

---

## Experience It

**Don't just receive a service. Experience it yourself.**

### Workshop — Engrave Your Own
**$100/person (2 hours)**

- Choose your material
- Design creation support
- Hands-on laser engraving
- Take home your creation

Families and groups welcome. Maui or Japan.

### Tour — See Off-Grid in Action
**Free (reservation required)**

- See actual off-grid setup
- Equipment walkthrough
- Q&A session

See for yourself that it really works.

### Build-Along — Construct Together
**$300/day**

- Join an off-grid construction
- Learn by doing
- Knowledge you can apply to your own setup

Don't just watch. Become capable.

---

## The Sato Family

| Member | Role | Contribution |
|---|---|---|
| **Takuya Sato** | Founder, Tech, Strategy | Off-grid construction, concept design, communication, handmade soap |
| **Mina** | Co-founder, Production, Marketing | Resin art, digital marketing |
| **Ito (5 years old)** | Soap helper, Heart of the family | Helps make handmade soap, symbol of family unity |

### Family Principles

1. **Stay together** — Family never separates
2. **Sleep together** — Live in the same place
3. **Secure infrastructure** — Ensure comfort
4. **Sustainable gift economy** — Freedom from monetary dependence

---

## FAQ

**Q: When can I stay in Japan?**
A: Jan–Mar, Jun–Jul, Nov–Dec (aligns with Hawaii school breaks)

**Q: Can I use a car?**
A: No vehicle lending for safety/insurance. Transportation available upon request.

**Q: Is the farmhouse comfortable?**
A: It's authentically old — but has Starlink Wi-Fi. 24-hour high-speed internet.

**Q: What's the donation guideline?**
A: $500–$1,000/week suggested. Not mandatory.

**Q: Off-grid construction outside Hawaii?**
A: Yes. Our methods are transferable — Japan, anywhere.

**Q: Can I just buy the equipment and do it myself?**
A: Yes. Everything in this catalog is public. Reach out if you have questions.

---

## Contact

- **Book:** calendly.com/pearlmemorial
- **Email:** business@satotakuya.jp
- **Instagram:** @pearl.memorial
- **Our story:** bit.ly/boundarist
- **Media:** press-kit.html
- **Partners:** Partner.pdf / Partner-ja.pdf

---

**Timeless Eternal Hope · Maui, Hawaii**

*In memory of Pearl (2007–2025)* 🐩

> "Cross boundaries through resonance."
> To exist. To act. To eternity.

> Everything in this catalog is open.
> Anyone, anywhere, can start.
>
> **It's your turn to become a Boundarist.**

*December 18, 2025 — The day the world begins to change.*
